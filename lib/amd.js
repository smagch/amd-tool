 ...
 *
 */
Amd.prototype.getData = function () {
  return {
    depMap: this._depMap
...
Amd.prototype.tree = function (name, done) {
  ...
  this.load(name, function (err) {
    if (err) {
      ...
      done(null, utils.getTree(name, data));
    }
  });
...
/**
 * pend dependencies
 * when all dependencies has resolved, execute the callback
 *
 * @param {Array}
 * @param {Function}
 * @api public
    ...
    if (self.has(name)) {
      ...
      self.load(name, function (err) {
        if (err) {
          ...
          self._errorMap[name] = err.toString();
          release();
        }
      });
    }
...
Amd.prototype.resolvePath = function (name) {
  var filePath = path.resolve(this.options.baseUrl, name);
  if (path.extname(filePath) === '') {
    filePath += '.js';
  }
  assert(!this._absMap[name]);
  this._absMap[name] = filePath;
  return filePath;
};
